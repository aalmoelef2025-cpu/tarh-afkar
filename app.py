...
def db():
    # تفعيل timeout لحماية النظام عند الضغط
    con = sqlite3.connect(DB_PATH, timeout=20.0)
    con.row_factory = sqlite3.Row
    con.execute("PRAGMA foreign_keys = ON")
    # تفعيل وضع WAL لتخزين قوي يسمح بالقراءة والكتابة في نفس الوقت
...
def init_db():
    ...
    con.execute("CREATE INDEX IF NOT EXISTS idx_hashtags_tag ON hashtags(tag)")
    con.execute("CREATE INDEX IF NOT EXISTS idx_article_hashtags_article ON article_hashtags(article_id)")
    con.execute("CREATE INDEX IF NOT EXISTS idx_article_hashtags_hashtag ON article_hashtags(hashtag_id)")
...
.logo{font-size:22px;color:#4f46e5}
.container{max-width:1050px;margin:20px auto;padding:12px}
...
.article-content{white-space:pre-wrap;line-height:1.9}
...
.avatar{width:58px;height:58px;border-radius:50%;object-fit:cover;background:#e2e8f0;border:2px solid #eef2ff}
...
.mine{background:#4f46e5;color:white;margin-right:auto}
.theirs{background:#f1f5f9;margin-left:auto}
...
.category-badge,.tag-badge{background:#14b8a6;color:white;border-radius:999px;padding:3px 8px;font-size:13px;font-weight:normal;display:inline-block;margin:3px}.tag-badge{background:#eef2ff;color:#3730a3}
...
def sync_article_hashtags(con, article_id, *texts):
    ...
    for tag in extract_hashtags(*texts):
        con.execute("INSERT OR IGNORE INTO hashtags(tag, created_at) VALUES(?, datetime('now','localtime'))", (tag,))
        hashtag_id = con.execute("SELECT id FROM hashtags WHERE tag=?", (tag,)).fetchone()["id"]
        con.execute("INSERT OR IGNORE INTO article_hashtags(article_id, hashtag_id) VALUES(?, ?)", (article_id, hashtag_id))
...
def make_backup(reason="auto"):
    ...
    name = f"tarh_afkar_{reason}_{stamp}.db"
    target = os.path.join(BACKUP_FOLDER, name)
    try:
        # استخدام النسخ الآمن الخاص بـ SQLite لتجنب تلف البيانات
        source_con = db()
...
def save_image(file_storage):
    ...
    if not original or not has_extension(original, IMAGE_EXTENSIONS):
        return "__BAD_TYPE__"
...
def save_video(file_storage):
    if not file_storage or not file_storage.filename:
        return None
    original = secure_filename(file_storage.filename)
    if not original or not has_extension(original, VIDEO_EXTENSIONS):
        return "__BAD_TYPE__"
    ext = original.rsplit(".", 1)[1].lower()
    new_name = f"{int(time.time())}_{secrets.token_hex(8)}.{ext}"
    file_storage.save(os.path.join(UPLOAD_FOLDER, new_name))
...
def home():
    ...
    # إضافة LIMIT 50 للحماية من انهيار الذاكرة عند كثرة المقالات
...
def public_profile(user_id):
    ...
    is_following = False
    if me:
        is_following = con.execute("SELECT 1 FROM follows WHERE follower_id=? AND following_id=?", (me["id"], user_id)).fetchone() is not None
    con.close()
...
def hashtag_page(tag):
    # حماية من ثغرات XSS وتنظيف الرابط
    ...
    return render_page("""
    <div class="container">
        <div class="card"><h2>منشورات الوسم #{{ tag }}</h2></div>
        {% for a in articles %}
        <div class="card">
            <h3><a href="/article/{{ a.id }}">{{ a.title }}</a> <span class="category-badge">{{ a.category }}</span></h3>
            ...
            {% if a.video %}<video controls preload="metadata"><source src="/uploads/{{ a.video }}"></video>{% endif %}
            <div class="actions"><a class="btn btn-light" href="/article/{{ a.id }}">قراءة كاملة</a></div>
        </div>
        {% else %}
        <div class="card">لا توجد منشورات بهذا الوسم.</div>
        {% endfor %}
    </div>
...
def save_article(article_id):
    ...
    if response:
        return response
    make_backup("before_save")
    con = db()
    article = con.execute("SELECT id FROM articles WHERE id=?", (article_id,)).fetchone()
    if not article:
        con.close()
        return message_page("غير موجود", "المقال غير موجود.", 404, True)
    existing = con.execute("SELECT 1 FROM saved_articles WHERE user_id=? AND article_id=?", (user["id"], article_id)).fetchone()
    if existing:
        con.execute("DELETE FROM saved_articles WHERE user_id=? AND article_id=?", (user["id"], article_id))
    else:
        con.execute("INSERT INTO saved_articles(user_id,article_id,created_at) VALUES(?,?,datetime('now','localtime'))", (user["id"], article_id))
    con.commit()
    con.close()
    return redirect(request.referrer or "/")
...
def saved_articles():
    ...
    <div class="container">
        ...
        {% else %}
        <div class="card">لا توجد مقالات محفوظة.</div>
        {% endfor %}
    </div>
    """, user=user, articles=articles)
...
def follow_user(user_id):
    ...
    if user_id == user["id"]:
        return redirect(request.referrer or "/")
    make_backup("before_follow")
    con = db()
    target = con.execute("SELECT id FROM users WHERE id=?", (user_id,)).fetchone()
    if not target:
        con.close()
        return message_page("غير موجود", "الحساب غير موجود.", 404, True)
    existing = con.execute("SELECT 1 FROM follows WHERE follower_id=? AND following_id=?", (user["id"], user_id)).fetchone()
    if existing:
        con.execute("DELETE FROM follows WHERE follower_id=? AND following_id=?", (user["id"], user_id))
    else:
        con.execute("INSERT INTO follows(follower_id, following_id, created_at) VALUES(?,?,datetime('now','localtime'))", (user["id"], user_id))
    con.commit()
    con.close()
...
def login():
    if request.method == "POST":
        ...
        # إضافة تأخير زمني لتدمير هجمات التخمين السريعة (Brute Force)
        time.sleep(1.5)
...
def users():
    ...
    if q:
        p = f"%{q}%"
        # إضافة LIMIT 100 لتسريع الصفحة
        rows = con.execute("SELECT * FROM users WHERE username LIKE ? OR full_name LIKE ? ORDER BY last_seen DESC LIMIT 100", (p, p)).fetchall()
    else:
        # إضافة LIMIT 100 لتسريع الصفحة
        rows = con.execute("SELECT * FROM users ORDER BY last_seen DESC, id DESC LIMIT 100").fetchall()
    con.close()
